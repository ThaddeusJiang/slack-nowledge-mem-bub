...
@pytest.fixture(autouse=True)
def _disable_live_mem(monkeypatch: pytest.MonkeyPatch) -> None:
    async def _capture_to_mem(**kwargs: object) -> None:
        return None

    monkeypatch.setattr("bub_slack.channel.capture_to_mem", _capture_to_mem)
    monkeypatch.setattr("bub_slack.channel.capture_slack_message", _capture_to_mem)
