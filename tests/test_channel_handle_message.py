...
async def test_plain_message_is_captured_without_entering_agent(
    channel, captured, monkeypatch
) -> None:
...
async def test_user_mentions_use_slack_display_name(
    channel, captured, monkeypatch
) -> None:
    captures: list[dict[str, Any]] = []
    ...
    event = _event(
        channel_type="im",
        text="<@UOTHER> latest PR?",
        ts="100.1",
    )

    await channel._handle_message(event)
    await channel._handle_message({**event, "ts": "100.2"})

    assert [item["text"] for item in captures] == [
        "@tj2 latest PR?",
        "@tj2 latest PR?",
    ]
    assert [item.content for item in captured] == [
        "@tj2 latest PR?",
        "@tj2 latest PR?",
    ]
    assert web.calls == 1
...
async def test_mention_is_captured_and_enters_agent(
    channel, captured, monkeypatch
) -> None:
    captures: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr("bub_slack.channel.capture_to_mem", _capture)
    event = _event(text="<@UBOT> recall this", ts="100.1")
    await channel._handle_message(event)

    assert captures == [event]
    assert len(captured) == 1
    assert captured[0].content == "recall this"
...
async def test_allow_channels_restriction_blocks_capture(
    on_receive, monkeypatch
) -> None:
    captures: list[dict[str, Any]] = []
    ...
    restricted._bot_user_id = "UBOT"
    await restricted._handle_message(_event(channel="C9", text="plain message"))
    assert captures == []
