...
this skill to answer. This Slack-specific rule overrides Bub's generic channel
instruction that says direct replies are ignored. Calling `slack_send.py` for a
normal reply causes a duplicate because Bub also posts the final answer. Use this
skill only for: proactive sends (another channel/thread), editing a bot message
you already posted, or reacting with an emoji.
