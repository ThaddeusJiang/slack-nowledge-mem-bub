...
class SlackChannel(Channel):
    ...
    def __init__(
        ...
        *,
        ...
        self._mem_client = NowledgeMemClient()
    ...
    async def _handle_message(self, event: dict[str, Any]) -> None:
        ...
        # Apply the same access policy to passive capture and agent turns. The
        # channel allow-list only constrains shared channels; DMs remain private
        # and are constrained only by the user allow-list.
        if (
            channel_type != "im"
        ...
        # Slack sends mentions as <@USER_ID>. Resolve them to the same display
        # names users see in Slack before storing or passing text to the agent.
        resolved_text = await self._resolve_user_mentions(text)

        # Every accepted human message reaches Mem. Capture is best-effort and
        # never raises, so a Mem outage cannot prevent an addressed agent turn.
        ...
        if self._bot_user_id:
            clean = await self._resolve_user_mentions(text.replace(mention, "").strip())
        else:
            clean = resolved_text
        if not clean:
            return
    ...
    async def _resolve_user_mentions(self, text: str) -> str:
        """Replace Slack <@USER_ID> tokens with current display names."""

        if self._web_client is None:
            return text

        user_ids = set(_USER_MENTION_RE.findall(text))
        for user_id in user_ids - self._user_names.keys():
            try:
                ...
                logger.opt(exception=True).debug(
                    "slack.users_info failed user_id={}", user_id
                )

        return _USER_MENTION_RE.sub(
            lambda match: (
                f"@{self._user_names[match.group(1)]}"
                if self._user_names.get(match.group(1))
                else match.group(0)
            ),
            text,
        )
...
# Slack user mentions, including the legacy optional ``|label`` form.
